...
def compute_internal_difference(components, mst):
    ...
    for component in components:
        ...
        max_weight = np.percentile([data[2] for data in edges], 75) if edges else 0
...
def segment_with_threshold(mst, k):
    ...
    max_weight = max([data['weight'] for u, v, data in mst.edges(data=True)], default = 0)
    scaled_k = k / max_weight if max_weight > 0 else k
    edges_to_remove = []
    ...
    mst.remove_edges_from(edges_to_remove)
    print(f"Removed {len(edges_to_remove)} edges for k={k}.")
    segments = list(nx.connected_components(mst))
